...
GPIB 계측기로 MLCC 특성(정전용량, 손실계수, ESR, 임피던스 등)을 측정하고,
...
로그인 다이얼로그
    └→ 메인 윈도우
         ├── 헤더: 앱 타이틀 | 계측기 연결 버튼 | 사용자명 + 로그아웃
         ├── 홈 화면: 측정 항목 카드 그리드 (6종)
         │     ├── 정전용량 (Cp)
         │     ├── ESR
         │     ├── Q Factor
         │     ├── 임피던스 (Z)
         │     ├── DC Bias 특성 → 전압 스윕 전용 페이지
         │     └── 온도 특성
         ├── DC Bias 측정 페이지 (전용)
         │     ├── 왼쪽: 주파수·AC Level·전압 범위·스텝·지연 시간 설정
         │     └── 오른쪽: 인가전압(V) × n차측정(F) 엑셀형 테이블 + CSV 내보내기
         └── 일반 측정 페이지 (Cp, ESR, Z 등)
               ├── 왼쪽: 측정 조건 + 계측기 선택
               └── 오른쪽: 실시간 측정값 + 이력 테이블
```
